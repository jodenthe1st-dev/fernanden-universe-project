...
const Podcasts = () => {
  ...
  const denseMinuteEpisodes = podcasts
    .filter((p) => p.tags.includes("dense-minute"))
    .slice(0, 4);

  const parenTipsEpisodes = podcasts
    .filter((p) => p.tags.includes("paren-tips"))
    .slice(0, 4);
  ...
  return (
    <Layout>
      <PageMeta
        title="Podcasts"
        description="Ecoutez les podcasts Fernanden : DENSE Minute (style en 60 secondes) et Paren'TIPS (conseils psychopedagogiques)."
      />
        ...
        <div className="container-main relative z-10">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
            ...
            <h1 className="heading-hero text-foreground mb-4">
              <AnimatedLetters text="Nos Podcasts" />
            </h1>
            <motion.p
            ...
            >
              Style, education et inspiration - decouvrez nos series audio
            </motion.p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            ...
            >
              ...
              <div className="p-5 space-y-3">
                {loading ? (
                  <div className="text-center py-8 text-muted-foreground">Chargement des episodes...</div>
                ) : denseMinuteEpisodes.length === 0 ? (
                  <div className="text-center py-8 text-muted-foreground">Aucun episode disponible pour le moment</div>
                ) : (
                  denseMinuteEpisodes.map((episode, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-heading font-semibold text-foreground truncate">{episode.title}</h4>
              ...
              <div className="bg-gradient-to-r from-cafee-mint to-cafee-mint/80 p-6 text-white">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-sm">Conseils psychopedagogiques</p>
                  </div>
                </div>
              </div>
      ...
      <section className="pb-16">
        <div className="container-main">
          ...
          >
            <GradientBlob className="top-0 left-0" color1="hsl(var(--primary) / 0.2)" size="200px" />

            <div className="relative z-10 text-center md:text-left">
              <h3 className="heading-subsection mb-2">Ne manquez aucun episode</h3>
              <p className="text-white/70">Decouvrez tous nos podcasts et restez informe</p>
            </div>
          </motion.div>
        </div>
      </section>
