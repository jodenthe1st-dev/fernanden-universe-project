...
import { Calendar, Clock, User, ArrowRight, Home, Loader2 } from "lucide-react";
...
const BlogSHE = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        setIsLoading(true);
        const allPosts = await BlogPostsService.getByCategory('she');
        setPosts(allPosts);
      } catch (error) {
        logger.error('Error fetching SHE blog posts:', error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    return (
      <Layout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <Loader2 className="w-12 h-12 animate-spin text-she-saffron" />
        </div>
      </Layout>
    );
  }
                      ...
                      <div className="lg:w-1/2 p-8 flex flex-col justify-center">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-muted-foreground">Marie fernanden</span>
                          </div>
